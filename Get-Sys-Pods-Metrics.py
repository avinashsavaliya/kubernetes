...
def write_to_excel(metrics_list):
    ...
    mydata = "Namespace,Active_Pod,CPU(%),Memory(MB)"
    ...
    for metrics in metrics_list:
        ...
        mydata = mydata + "\n" + myrow
        row += 1
    filename = 'syspods_'+str(dt.now()).split('.')[0].replace(" ","T").replace(":","_")
    wb.save(filename+'.xls')
    with open(filename+".txt","w") as fp:
        fp.write(mydata)
    with open("syspods.txt","w") as fp:
        fp.write(mydata)
    ...
    for ns in ns_list:
        ...
        for pod in active_pods:
            try:
                ...
                print("Couldn't get metrics for pod "+pod+" : "+str(e))
